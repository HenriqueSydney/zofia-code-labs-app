...
describe("UpdateInvoiceUseCase", () => {
  beforeEach(() => {
    ...
    sut = new UpdateInvoiceUseCase(invoiceRepository);
  });

  it("deve atualizar fatura existente", async () => {
    const invoice = await seedInvoice();

    await sut.execute({
      id: invoice.id,
      userId: randomUUID(),
      data: { description: "Parcela atualizada" },
    });

    expect(invoiceRepository.items[0].description).toBe("Parcela atualizada");
  });

  it("não deve atualizar fatura inexistente", async () => {
    await expect(() =>
      sut.execute({
        id: randomUUID(),
        userId: randomUUID(),
